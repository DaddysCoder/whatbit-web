...
/**
 * Adapter between the ported wizard and this app's own backend
 * (`/api/assessment/[token]`) — replaces the standalone SPA's
...
function endpoint(token: string) {
  return `/api/assessment/${encodeURIComponent(token)}`;
}
